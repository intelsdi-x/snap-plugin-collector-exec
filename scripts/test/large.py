...
class ExecCollectorLargeTest(unittest.TestCase):
    ...
    def test_exec_collector_plugin(self):
        # load exec collector
        loaded = self.binaries.snapctl.load_plugin("snap-plugin-collector-exec")
        self.assertTrue(loaded, "exec collector loaded")
        ...
        rules = self.binaries.snapctl.metric_get("/intel/exec/metric0")
        self.assertEqual(len(rules), 1, "Rules available {} expected {}".format(len(rules), 1))

        # create and list available task
        task_id = self.binaries.snapctl.create_task("/snap-plugin-collector-exec/scripts/docker/large/task.json")
        tasks = self.binaries.snapctl.list_tasks()
        ...
        self.binaries.snapctl.unload_plugin("collector", "exec", "1")
        metrics = self.binaries.snapctl.list_metrics()
        plugins = self.binaries.snapctl.list_plugins()
...
if __name__ == "__main__":
    test_suite = unittest.TestLoader().loadTestsFromTestCase(ExecCollectorLargeTest)
