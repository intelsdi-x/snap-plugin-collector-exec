...
PLUGIN_URL = "http://snap.ci.snap-telemetry.io/plugin/build/latest/snap-plugin-collector-exec"
...
class Snapd(Binary, threading.Thread):
    ...
    def run(self):
        config_path = os.path.join('/', 'snap-plugin-collector-exec', 'examples', 'configs', 'config.json')
        cmd = '{} --config {} -t 0 -l 1 '.format(os.path.join(self.dir, self.name), config_path)
...
class Snapctl(Binary):
    ...
    def unload_plugin(self, plugin_type, plugin_name, plugin_version):
        cmd = '{} plugin unload {}:{}:{}'.format(os.path.join(self.dir, self.name), plugin_type, plugin_name,
                                                 plugin_version)
